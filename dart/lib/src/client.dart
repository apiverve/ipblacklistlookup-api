...
class IpblacklistlookupValidationException implements Exception {
  final List<String> errors;

  IpblacklistlookupValidationException(this.errors);

  @override
  String toString() => 'IpblacklistlookupValidationException: ${errors.join("; ")}';
}
...
/// IP Blacklist Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/ipblacklistlookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ip] (required) - The IP address to check against blocklists [format: ip]
class IpblacklistlookupClient {
  final String apiKey;
  ...
  IpblacklistlookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/ipblacklistlookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [IpblacklistlookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw IpblacklistlookupValidationException(errors);
    }
  }

  /// Execute a request to the IP Blacklist Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<IpblacklistlookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw IpblacklistlookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return IpblacklistlookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw IpblacklistlookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw IpblacklistlookupException('Resource not found');
      } else {
        throw IpblacklistlookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is IpblacklistlookupException) rethrow;
      throw IpblacklistlookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the IP Blacklist Lookup API client.
class IpblacklistlookupException implements Exception {
  final String message;

  IpblacklistlookupException(this.message);

  @override
  String toString() => 'IpblacklistlookupException: $message';
}
