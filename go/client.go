// Package ipblacklistlookup provides a Go client for the IP Blacklist Lookup API.
//
// For more information, visit: https://apiverve.com/marketplace/ipblacklistlookup?utm_source=go&utm_medium=readme
package ipblacklistlookup
...
const (
	baseURL        = "https://api.apiverve.com/v1/ipblacklistlookup"
	defaultTimeout = 30 * time.Second
)

// Client is the IP Blacklist Lookup API client.
type Client struct {
	apiKey     string
	httpClient *resty.Client
}

// NewClient creates a new IP Blacklist Lookup API client.
...
// Execute makes a request to the IP Blacklist Lookup API with typed parameters.
//
...
//   - ip (required): string - The IP address to check against blocklists [format: ip]
